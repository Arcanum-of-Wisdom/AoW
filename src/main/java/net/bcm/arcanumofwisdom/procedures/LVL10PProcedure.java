...
public class LVL10PProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		boolean lvl10active = false;
		if ((entity instanceof Player _plr ? _plr.experienceLevel : 0) == 10 && entity.getData(ArcanumOfWisdomModVariables.PLAYER_VARIABLES).wisdom < 10) {
			{
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles(ParticleTypes.COMPOSTER, x, y, z, 15, 0, 0, 0, 0.5);
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(ArcanumOfWisdomModMobEffects.WISDOM.get(), (int) Double.POSITIVE_INFINITY, 10, false, false));
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
