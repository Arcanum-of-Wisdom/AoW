...
public class LVL30Procedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		boolean lvl30active = false;
		if ((entity instanceof Player _plr ? _plr.experienceLevel : 0) == 20 && entity.getData(ArcanumOfWisdomModVariables.PLAYER_VARIABLES).wisdom < 20) {
			{
			...
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(ArcanumOfWisdomModMobEffects.WISDOM.get(), (int) Double.POSITIVE_INFINITY, 20, false, false));
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
