...
public record SkillTreeButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static final Type<SkillTreeButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(ArcanumOfWisdomMod.MODID, "skill_tree_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, SkillTreeButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, SkillTreeButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
	}, (RegistryFriendlyByteBuf buffer) -> new SkillTreeButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<SkillTreeButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final SkillTreeButtonMessage message, final IPayloadContext context) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		ArcanumOfWisdomMod.addNetworkMessage(SkillTreeButtonMessage.TYPE, SkillTreeButtonMessage.STREAM_CODEC, SkillTreeButtonMessage::handleData);
	}
}
