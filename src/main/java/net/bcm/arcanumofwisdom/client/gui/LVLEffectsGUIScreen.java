...
import net.bcm.arcanumofwisdom.procedures.LVLEffectsOnStatusProcedure;
import net.bcm.arcanumofwisdom.procedures.LVLEffectsOnDisplayConditionProcedure;
import net.bcm.arcanumofwisdom.procedures.LVLEffectsOffStatusProcedure;
...
public class LVLEffectsGUIScreen extends AbstractContainerScreen<LVLEffectsGUIMenu> {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("arcanum_of_wisdom:textures/screens/lvl_effects_gui.png");
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
		...
		guiGraphics.blit(ResourceLocation.parse("arcanum_of_wisdom:textures/screens/skilltree2.png"), this.leftPos + -202, this.topPos + -105, 0, 0, 400, 200, 400, 200);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		if (LVLEffectsWarningDisplayConditionProcedure.execute(world))
			guiGraphics.drawString(this.font, Component.translatable("gui.arcanum_of_wisdom.lvl_effects_gui.label_empty"), -101, 58, -65536, false);
		...
		if (LVLEffectsOnStatusProcedure.execute(world, entity))
			guiGraphics.drawString(this.font, Component.translatable("gui.arcanum_of_wisdom.lvl_effects_gui.label_on"), -168, -23, -16711885, false);
		if (LVLEffectsOffStatusProcedure.execute(world, entity))
			guiGraphics.drawString(this.font, Component.translatable("gui.arcanum_of_wisdom.lvl_effects_gui.label_off"), -168, -23, -65485, false);
	}
	...
	public void init() {
		...
		button_off = new PlainTextButton(this.leftPos + -132, this.topPos + -60, 40, 20, Component.translatable("gui.arcanum_of_wisdom.lvl_effects_gui.button_off"), e -> {
			if (LVLEffectsOffDisplayConditionProcedure.execute(world, entity)) {
				PacketDistributor.sendToServer(new LVLEffectsGUIButtonMessage(0, x, y, z));
				LVLEffectsGUIButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}, this.font) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int gx, int gy, float ticks) {
				this.visible = LVLEffectsOffDisplayConditionProcedure.execute(world, entity);
		...
		button_on = new PlainTextButton(this.leftPos + -168, this.topPos + -60, 35, 20, Component.translatable("gui.arcanum_of_wisdom.lvl_effects_gui.button_on"), e -> {
			if (LVLEffectsOnDisplayConditionProcedure.execute(world, entity)) {
				PacketDistributor.sendToServer(new LVLEffectsGUIButtonMessage(1, x, y, z));
				LVLEffectsGUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}, this.font) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int gx, int gy, float ticks) {
				this.visible = LVLEffectsOnDisplayConditionProcedure.execute(world, entity);
				super.renderWidget(guiGraphics, gx, gy, ticks);
			}
		};
		...
		imagebutton_buttonred = new ImageButton(this.leftPos + -18, this.topPos + -120, 32, 32,
				new WidgetSprites(ResourceLocation.parse("arcanum_of_wisdom:textures/screens/buttonred.png"), ResourceLocation.parse("arcanum_of_wisdom:textures/screens/buttonredhighlight.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new LVLEffectsGUIButtonMessage(2, x, y, z));
